...
def generate_moving_mnist(shape=(64,64), seq_len=30, seqs=10000, num_sz=28, nums_per_image=1, fast=True):
    mnist = load_dataset()
    width, height = shape
    ...
    for seq_idx in xrange(seqs):
        ...
        speeds = np.random.randint(5, size=nums_per_image)
        speeds += 10 if fast else 2
        ...
        for frame_idx in xrange(seq_len):
            ...
            for i, pos in enumerate(next_pos):
            	if pos[0] < -2 or pos[0] > lims[0]+2 or pos[1] < -2 or pos[1] > lims[1]+2:
            		temp_veloc = list(veloc[i])
            		temp_veloc = [-v for v in temp_veloc]
            		veloc[i] = tuple(temp_veloc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dest', type=str, default='.', dest='dest')
    parser.add_argument('--filetype', type=str, default='jpg', dest='filetype')
    parser.add_argument('--frame_size', type=int, default=128, dest='frame_size')
    parser.add_argument('--seq_len', type=int, default=100, dest='seq_len') # length of each sequence
    parser.add_argument('--seqs', type=int, default=1, dest='seqs') # number of sequences to generate
    parser.add_argument('--num_sz', type=int, default=28, dest='num_sz') # size of mnist digit within frame
    parser.add_argument('--nums_per_image', type=int, default=1, dest='nums_per_image') # number of digits in each frame
