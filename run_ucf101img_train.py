...
def train_model(exp_id,
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      #for v in variables:
      #  print(v)
      #variables_to_restore = [v for v in variables if (v.name.split('/')[1]!='logit' and v.name.split('/')[2]!='logit' and v.name.split('/')[1]!='learn_rate:0')]
      variables_to_restore = []
      for v in variables:
        if v.name.split('/')[1]!='logit':
          if len(v.name.split('/')) > 2: 
            if v.name.split('/')[2]!='logit':
      ...
      variables_to_init = []
      for v in variables:
        if v.name.split('/')[1] == 'logit' or (len(v.name.split('/')) > 2 and v.name.split('/')[2] == 'logit'):
...
def main():
  ...
  train_data = get_dataset(dataset_name, train_str, config=config)
  ...
  test_data = get_dataset(
      dataset_name, test_str, data_aug=False, cycle=False, prefetch=False, config=config)
